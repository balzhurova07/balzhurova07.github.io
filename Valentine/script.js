...
if (document.body.id === 'page-step2') {
    ...
    const warningPhrases = [
        ...
        'Ты хорошо подумал?',
        'Только {percent}%?',
        'Может хорошенько подумаем?',
        'Подумай еще',
        'Твой окончательный ответ {percent}%?',
        '{percent}% — ох, ну раз уж это твой выбор',
        'Ты уверен, что не хочешь подумать еще?'
    ];

    function updateLoveWarning(value) {
        if (value < 40) {   // порог 70% – можно изменить
